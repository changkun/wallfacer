...
func (h *Handler) GitHubAuthConnect(w http.ResponseWriter, _ *http.Request) {
	if h.github == nil || h.github.Broker == nil {
		http.Error(w, "github connect not available", http.StatusServiceUnavailable)
		return
	}
	// The live brokered flow is added with the ../auth GitHub App capability;
	// the broker seam is in place so this lights up without further wiring here.
	http.Error(w, "github connect not yet implemented", http.StatusNotImplemented)
}
