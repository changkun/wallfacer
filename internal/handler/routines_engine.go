...
func (h *Handler) StartRoutineEngine(ctx context.Context) {
	...
	watcher.Start(ctx, watcher.Config{
		// A resubscribing wake source follows the active workspace group
		// across switches. Using h.store here would pin the watcher to
		// the initial store forever — archives/deletes in later groups
		// would never wake the reconciler and the engine would keep
		// firing stale routine entries.
...
// runs with: the routine card's own prompt verbatim.
