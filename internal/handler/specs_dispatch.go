...
func (h *Handler) SpecTransition(w http.ResponseWriter, r *http.Request) {
	...
	case "migrate":
		h.MigrateSpec(w, r)
	default:
		http.Error(w, "action must be one of: dispatch, undispatch, archive, unarchive, validate, stale, unstale, dismiss-stale, force-complete, migrate", http.StatusBadRequest)
	}
}
...
// SpecCompletionHook is defined in specs_drift.go (it now drives the task-done
// drift pipeline in addition to the historical complete-on-done behavior).
