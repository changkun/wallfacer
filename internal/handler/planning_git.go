...
// workspace with a message of the form `plan: round N — <summary>`. Returns
// nil (not an error) when specs/ has no pending changes or when the git
// status probe itself fails — planning commits are best-effort and must not
// block the HTTP response.
//
// Round N is derived by counting existing `plan: round` commits reachable
// from HEAD and adding one. This is monotonically increasing even after undo
// operations, since reverted commits no longer appear in the log.
func commitPlanningRound(ctx context.Context, ws, summary string) error {
	...
	if err := cmdexec.Git(ws, "add", "specs/").WithContext(ctx).Run(); err != nil {
		return fmt.Errorf("git add specs/: %w", err)
	}
