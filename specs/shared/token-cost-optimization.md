...
1. *With sandbox hooks*: The hook HTTP server's `PreToolUse` handler rewrites Bash commands with RTK compression pipes. Controlled by `WALLFACER_HOOKS_COMPRESSION` env var.
...
**Alternative via sandbox hooks**: If Claude Code lifecycle hooks are enabled, budget warnings can instead be injected through the `UserPromptSubmit` hook's `additionalContext` field — exactly as cc-budget does. This is more natural (the agent sees it as part of the prompt context, not as a continuation message) but requires the hooks infrastructure. The runner-side approach above works without hooks and serves as the default.
