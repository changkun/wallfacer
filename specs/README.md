...
| [workspace-model.md](local/workspace-model.md) | Drafted | **Foundation redesign.** Decouple a workspace's identity (stable UUID) from its membership (a mutable folder set), owned by a personal user or org. Retires the "identity = hash of sorted paths" model (`InstructionsKey`) that orphans all scoped data (`data/<key>/`) on any folder edit; introduces a stable `DataKey` storage handle so migration moves zero bytes and the 9 live groups plus 17 orphaned data dirs (non-empty adopted as dormant) carry forward. Renames `Group`→`Workspace` across Go/API/Vue. Cloud workspace objects and git-repo association deferred. Non-leaf; breakdown pending. |
