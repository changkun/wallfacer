...
| [topos-live-agent-trace.md](local/topos-live-agent-trace.md) | Drafted | Live agent traces for topos runs: expose topos's internal event bus through a root `Options.Observer` seam + an assistant-text event (SDK change), forward through the agentgraph seam to a per-task trace, stream it over SSE, and animate the lineage nodes + per-agent transcript live in `AgentLineage.vue`. Closes the one multi-agent surface with no live view. |
