...
| ↳↳ [dedicated-chat-ui.md](spec-coordination/spec-coordination/spec-planning-ux/dedicated-chat-ui.md) | Drafted | Promote chat to a first-class Claude-style surface: sidebar leads with Chat/Spec/Board, a dedicated `/chat` view with a session sub-sidebar and entry screen (replaces chat-first), and a floating draggable chat popup in spec mode. Frontend-only re-layout reusing the existing thread model; one extracted chat core, two mounts. |
...
| [free-form-specs.md](local/free-form-specs.md) | Drafted | Render frontmatter-less markdown specs read-only in spec mode (today they silently collapse the tree to empty), plus a non-blocking, dismissible suggestion to migrate them into wallfacer lifecycle-managed specs via opt-in frontmatter scaffolding. |
