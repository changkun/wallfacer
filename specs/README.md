...
| [backend-redundancy-cleanup.md](local/backend-redundancy-cleanup.md) | Drafted | Umbrella for follow-ups to the June 2026 pass-1 cleanup. Split into 5 backend-only leaves (TaskUsage cache fix, handler helper dedup, background launcher, cache wrappers inline, generic Registry+CRUD) that can run anytime, and 6 API-surface leaves (planning threads, spec actions, oversight phase, task actions, auth orgs, ideate facade) that block on vue-frontend-migration. |
