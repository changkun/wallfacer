...
| [harness-abstraction.md](shared/harness-abstraction.md) | Validated | Both | `internal/harness/` with a `Harness` interface (BuildArgv / ParseEvent / AuthEnv / Capabilities). Interface + Claude/Codex migration **shipped**; Pi **shipped**; Cursor, OpenCode in progress. Topos is a remote *executor*, not a harness. |
