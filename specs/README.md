...
| [harness-abstraction.md](shared/harness-abstraction.md) | Validated | Both | `internal/harness/` with a `Harness` interface (BuildArgv / ParseEvent / AuthEnv / Capabilities). Interface + Claude/Codex migration **shipped**; Cursor, OpenCode, Pi **shipped**. Topos is a remote *executor*, not a harness. |
