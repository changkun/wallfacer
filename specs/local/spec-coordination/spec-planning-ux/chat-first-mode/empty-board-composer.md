...
- Kanban columns are hidden while the composer is mounted via `#board:has(#board-empty-composer:not(:empty)) > [id^="col-wrapper-"] { display: none }` so the empty board shows only the composer instead of four empty column headers alongside it.
- The DOM structure mirrors the Plan-mode chat composer: an outer `.board-composer-wrap` card (same border/radius/shadow/bg-card as `.spec-chat-stream` in chat-first) wraps an inset `.board-composer.spec-chat-composer` that reuses the Slack-style base from `spec-mode.css` (bg-input, 6/8/8 margin, 8px radius) with zero board-specific overrides on the shell. The action bar exposes three `.spec-chat-composer__action` icons — `/` (opens the templates picker, the board-mode analogue of Plan's slash-command menu), `@` (inserts an `@` and re-fires the input event so the mention autocomplete opens), and `▾` (toggles the advanced panel) — plus a single `➤` send button (no split group, since Board has no send-mode toggle). Advanced exposes sandbox, timeout, and goal; `fresh_start` is omitted because it is only meaningful when resuming a previous session. An empty-state hint reusing `.spec-chat-empty-hint` / `.spec-chat-empty-hint__cmd` sits above the inner composer, and the "Planning something larger?" bridge line sits below the card as a sibling (outside the card framing).
