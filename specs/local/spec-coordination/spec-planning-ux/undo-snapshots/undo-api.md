...
- **Handler lives in `internal/handler/planning_undo.go`** (new file). Keeps
  the HTTP + helpers + regex together and leaves `planning.go` focused on the
  message-send path. Matches the repo convention of one file per concern.
- **Route wiring**: the spec's "Register in `server.go`" referred to a
  top-level `server.go` that does not exist. Actual wiring is in
  `internal/cli/server.go` — one entry in the `handler-name → handler-func`
  map (line 712) plus one entry in the body-limit map (line 828). Routes
  themselves are declared in `internal/apicontract/routes.go` and the JS
  helper is regenerated by `make api-contract`.
- **Workspace accessor**: the spec's `h.cfg.Workspaces` does not exist on
  the `Handler` struct. The active workspace list is read through
  `h.currentWorkspaces()` which acquires `h.snapshotMu.RLock` and returns
  a clone — safe for handler goroutines.
- **`cancelDispatchedTask` instead of reusing `UndispatchSpecs`**: the
  spec proposed `undispatchByTaskID` delegating to the existing
  `UndispatchSpecs` handler, but that handler re-parses the spec file
  looking for `dispatched_task_id` — which `git reset --hard` has already
  removed. The new private helper on `Handler` does only the store-side
  work: `GetTask`, skip if already terminal, `CancelTask`, emit a
  state-change event via `insertEventOrLog`. Best-effort and
  log-only on every error path.
- **Safety check — planning commit must be at HEAD.** Added a `git rev-parse
  HEAD` compare before the reset. If the found planning commit is not at
  HEAD (user committed something else since), the endpoint returns 409 with
  a clear message rather than blindly running `git reset --hard HEAD~1` and
  destroying the intermediate commit. Deviates from the spec's literal
  pseudo-code but protects against data loss.
- **Em-dash regex tolerance**: git's default `--cleanup=strip` trims trailing
  whitespace from commit subjects, so a summary-less commit ("plan: round 42 — ")
  lands on disk as "plan: round 42 —". The subject parser allows both
  forms via `(?:\s+—\s*(.*))?$` instead of requiring a literal " — " with
  flanking spaces.
- **Response adds a `workspace` field** on top of the spec's shape. When
  multiple workspaces exist, the caller needs to know which one was
  reverted; adding the field is cheap and forward-compatible with the
  deferred multi-workspace undo.
- **UUID extraction uses a single regex** instead of iterating lines. The
  pattern (`(?m)^\+dispatched_task_id:\s*<UUID>`) implicitly skips
  diff-header lines like `+++ b/specs/foo.md` (those don't start with
  `+dispatched_task_id`) and the dedupe is a simple `seen` map.
- **Tests**: six Go tests cover the five spec cases plus `TestParsePlanSubject`
  for the subject parser (worth testing independently because the regex had
  to tolerate git's whitespace trimming — one table-driven case caught it).
  Reuses the `initPlanningTestRepo` / `writeSpec` / `runGit` / `gitLogSubjects`
  helpers from `planning_git_test.go`.
