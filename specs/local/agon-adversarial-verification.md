...
depends_on: []
affects:
  - internal/adversarial/verifier.go
...
### The `Verifier` interface (`internal/adversarial/verifier.go`)
...
// Verifier is the plugin seam for post-run adversarial verification.
// The zero implementation (NoopVerifier) returns (nil, nil) immediately.
// Future implementations may wire in different debate protocols or harnesses.
type Verifier interface {
    // Verify runs adversarial verification against the provided input.
    // Returns nil when the verifier is disabled, the task lacks a session
    // ID, or the diff is trivial.
    Verify(ctx context.Context, in Input) (*Result, error)
}

// Input parameterizes one verification run.
type Input struct {
    TaskPrompt    string  // task description
    Criteria      string  // from Task.Criteria; empty means no acceptance bar
    SessionID     string  // from Task.SessionID; required for the proposer path
    DiffPatch     string  // pre-computed git diff (generated by the runner)
    Cwd           string  // working directory (worktree path)
    StateDir      string  // directory for agon state (.agon/sessions/)
    ForkCount     int     // number of critic forks (default 2)
    MaxRounds     int     // debate rounds per fork (default 4)
    CostCapTokens int     // soft token budget (default 50000)
}

// Result is what a Verify call produced.
type Result struct {
    Unresolved int    // number of open attacks that were not conceded or rebutted
    Headline   string // markdown summary of the highest-contention unresolved attack
    SessionDir string // path to .agon/sessions/<id>/
    USD        float64
}
```

**`internal/adversarial/noop.go`**: `NoopVerifier` — `Verify` returns `(nil, nil)`
immediately. This is the active implementation when agon is toggled off, ensuring
zero cost and zero behavioral change for the skip path.
...
`AgonVerifier` is the concrete `Verifier` implementation. It wires the proposer and
critic into an `adversarial.Engine` and calls `Run`:
...
func (v *AgonVerifier) Verify(ctx context.Context, in Input) (*Result, error) {
    if in.SessionID == "" {
...
**Phase 1 — package skeleton** (`internal/adversarial/`). Define `Verifier`,
`Input`, `Result`, `NoopVerifier`. Compile against a stub
`pkg/adversarial` (can be an empty package with just the interfaces, until agon
spec 37 lands). Tests: `NoopVerifier.Verify` returns nil; `AgonVerifier` with a
mock engine returns the correct `Result` fields.
