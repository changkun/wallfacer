...
3. **User-defined commands.** Out of scope here, but the registry should leave room for it. The runtime-discoverable-prompt direction this once overlapped with (the archived [extensible-prompts.md](../.archive/shared/extensible-prompts.md)) now lives in the Topos agent-document model; a future user-defined-command path would integrate there rather than in a wallfacer-local prompt registry.
