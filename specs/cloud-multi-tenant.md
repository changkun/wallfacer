...
- Container execution (via sandbox executor — see `cloud-sandbox-executor.md`)
- Data directory (backed by cloud storage — see `cloud-data-storage.md`)
...
- Sandbox executor config (see `cloud-sandbox-executor.md`)
- Data directory (per-user, backed by cloud storage — see `cloud-data-storage.md`)
...
| Cloud storage backend | `internal/store/` | See `cloud-data-storage.md` |
| Remote sandbox executor | `internal/runner/executor.go` | See `cloud-sandbox-executor.md` |
...
3. Wallfacer loads state from cloud storage (see `cloud-data-storage.md`)
...
- **Cloud Data Storage** (`cloud-data-storage.md`): Required for hibernate/wake — instance state must survive process restarts. Without cloud storage, instances lose all task data on restart.
- **Sandbox Executor** (`cloud-sandbox-executor.md`): Required if instances should not run containers locally. Without it, each instance needs a local container runtime (Docker socket mount), which works but limits density.
