...
This spec lives at the top level of `specs/` rather than inside a track directory because it's a one-off devops migration, not a product feature. It does **not** appear in `specs/README.md`'s roadmap; the umbrella ([[local-build-deploy]] in `terraform/specs/`) is the index.
