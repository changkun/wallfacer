...
with YAML frontmatter) and felt out of place next to board task commits.

Revised format now matches the board commit voice:
...
- The subject and body are generated by the **same** commit-message agent
  that board tasks use. `Runner.GenerateCommitMessage(ctx, data)` was added
