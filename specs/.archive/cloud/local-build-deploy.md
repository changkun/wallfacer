...
This spec lives in the Cloud Platform track (`specs/cloud/`), alongside `cloud-infrastructure.md`, because it concerns deploying the `wallfacerd` server image. It is archived (shown when "Show archived" is on). The cross-repo umbrella that indexes the multi-repo migration lives in the platform infrastructure spec tree; the former `depends_on` edges to sibling repos were dropped from the frontmatter because the wallfacer validator cannot resolve cross-repo paths.
