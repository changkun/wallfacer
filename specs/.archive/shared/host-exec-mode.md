...
This spec is **not** a replacement for the native sandbox backends (Bubblewrap / Landlock / `sandbox_init` / JobObjects / Hyper-V) tracked in Cella, the Latere sandbox runtime, in its per-platform native-backend specs. Those specs treat host execution as a platform for adding a new isolation layer. This spec owns the simpler "no isolation at all" case those specs mention as "Option C: Local CLI" but do not implement.
...
**Non-goal.** Host mode intentionally provides no write containment — an agent can `rm -rf $HOME`. This is the deliberate tradeoff. The mitigation is documentation + the Settings UI banner above. Users who need containment should use container mode, or wait for Cella's native-OS backends to ship. No code-level defense in v1.
...
- **No write containment**: intentional. The Settings UI banner, doctor output, and commit messages all make this explicit. Users who need containment should use container mode or Cella's in-progress native-OS backends.
