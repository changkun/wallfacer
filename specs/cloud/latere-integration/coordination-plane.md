...
and defaults **on for a signed-in instance** (deliberate product decision,
revising the earlier off-by-default; anonymous instances still connect to
nothing, the data-boundary floor; opt out with `WALLFACER_COORDINATION=0` or the
in-app toggle). The gate governs each user's `wallfacer run`, not the
`wallfacer web` coordinator pod. See `coordination-plane/spec-comments.md` Outcome for the
