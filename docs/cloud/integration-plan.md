...
> **Update since 2026-05-30.** Several items proposed here have since SHIPPED
> (reconciling with `cloud/README.md`, whose Roadmap marks them as Phase 2):
> - **JWT OptionalAuth middleware** wired on `/api/*` (`internal/cli/server.go:399`,
>   `auth.OptionalAuth(jwtValidator, srvHandler)`).
> - **ForceLogin** redirect for anonymous HTML navigation in cloud mode
>   (`internal/cli/server.go:396`).
> - **RequireSuperadmin** gating on admin routes (`internal/cli/server.go:921`).
>
> Still UNBUILT: the proposed `internal/cloudsync` metadata-sync adapter
> (Part 3). No such directory exists in the tree. The sink interface, offline
> queue, and cloud metadata service remain a proposal.
...
- **The integration seams are already interfaces**, selected by config:
  ...
  - `handler.AuthProvider` (`internal/handler/login.go:13`): swappable; nil means
...
| **pkg** | `latere.ai/x/pkg` (wallfacer already imports v0.22.0) | `jwtauth`, `oidc`, `authkit`, `scopes` (already has wallfacer scopes), `otel`, `audit`, `md` | Live |
...
| **terraform** | DOKS Frankfurt | `latere` namespace, OTEL to ClickHouse to Grafana chain (Grafana already has wallfacer filters); **no wallfacer module yet** | Deployed |
...
| `identity/agent-token-exchange.md` | drafted | **DEFER (Phase 3)** | Only needed when agents call FS/telemetry from sandboxes, i.e. the Cella execution phase. RFC 8693 endpoint already exists in auth. |
...
> **Unbuilt as of this update.** The `internal/cloudsync` package below does not
> exist in the tree yet. This part remains a proposal.
...
- **Tap point:** the existing event-sourcing stream (`store.TaskEvent`). One
  subscriber maps each event to a `MetadataEvent` through the **data-boundary
  allowlist** (Part 2), dropping everything forbidden.
- **Offline durability:** bounded local queue (the `queue-backpressure` spec:
  cap ~10k, drop-oldest, exponential-backoff retry). Cloud outage leaves local
...
- **Selection:** `WALLFACER_CLOUD=true` + `AUTH_*` selects the real sink; else nil
  sink (no-op). Mirrors how `AuthProvider`/`jwtValidator` are already wired.
...
seam (path-hash to cloud identity), to design then, not now.
...
- **Phase 2: Cloud v1 (do-now candidate).** Metadata-sync adapter (sink +
  ...
  FS, no per-tenant instances.* (Note: the JWT/ForceLogin/superadmin pieces of
  Phase 2 have already shipped; see the top update.)
