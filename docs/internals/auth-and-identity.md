...
- **OptionalAuth**: validates an `Authorization: Bearer <jwt>` against the auth service's JWKS and injects the identity on success; missing, malformed, or expired tokens pass through anonymous rather than 401. It runs downstream of CookieAuth and overwrites the context identity, so a JWT wins when both are present. `BuildValidator` accepts only this service's audience, `AUTH_AUDIENCE` (default: the client id), never the issuer: a login token is addressed to the issuer alone and opens nothing here. The credential a caller presents is the actor token auth mints for that audience; a local instance mints one from its session for every dial of the coordination connector (`coordinationMinter`), and the login token itself goes to the issuer only. `AUTH_JWKS_URL` and `AUTH_ISSUER` override the derived defaults, and an unset issuer skips the `iss` check.
- **BearerAuth** (`handler.BearerAuthMiddleware`): the `WALLFACER_SERVER_API_KEY` static gate. With no key configured it is a no-op. With a key, every request must present `Authorization: Bearer <key>`, except: requests already carrying an identity from the cookie or JWT path bypass the check (so a cookie-only browser works in a deployment that also sets the key for scripts), SSE/WebSocket paths accept `?token=<key>` because `EventSource` cannot set headers, and `GET /` passes so the SPA shell loads. Key comparisons are constant-time. A local run (`WALLFACER_CLOUD` unset) never has the gate off: when no key is configured the server generates one at startup, persists it at `<configDir>/server-api-key` (mode 0600, outside the `.env` that agent processes receive) and logs the path. The SPA shell embeds the key only for a loopback peer or a request that already presents it (`Authorization: Bearer` or `?token=`); any other caller gets the shell with an empty `serverApiKey`, so the public `GET /` no longer hands the key to anonymous hosts.
...
- **Superadmin gating.** `POST /api/admin/rebuild-index` is wrapped by `auth.RequireSuperadmin` whenever an OIDC client is wired (`adminOnly` in `BuildMux`, keyed on `h.HasAuth()`, which auth-by-default makes true on every run): anonymous callers get 401, signed-in non-superadmins 403, and only a session whose `is_superadmin` claim is true reaches the handler.
- **Sandbox trust-plane proxy.** `/internal/sandbox-proxy/llm/anthropic/*` and `/internal/sandbox-proxy/llm/openai/*` (`internal/handler/sandbox_proxy.go`) let cloud sandboxes reach LLM providers without holding real credentials. Configuration is the provider keys; the routes answer 503 until at least one is set, which is the permanent local-mode state. Inbound requests carry a sandbox JWT with `aud=wallfacer-sandbox-proxy` and the scope `llm:proxy`; the proxy substitutes the real provider key (`x-api-key` for Anthropic, `Authorization: Bearer` for OpenAI). The GitHub-token route went with auth's installation-token brokering (2026-09-13): a sandbox that needs git credentials gets them the way any client does, from the person's own GitHub integration.
...
| `AUTH_AUDIENCE` | `AUTH_CLIENT_ID` | The only `aud` the API validator accepts; callers present an actor token minted for it. Nothing is requested on `/authorize` |
| `WALLFACER_SERVER_API_KEY` | local: generated at `<configDir>/server-api-key`; cloud: unset (gate off) | Static bearer for programmatic access |
