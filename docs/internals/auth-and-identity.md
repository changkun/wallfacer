...
- **Superadmin gating.** `POST /api/admin/rebuild-index` is wrapped by `auth.RequireSuperadmin` whenever an OIDC client is wired (`adminOnly` in `BuildMux`, keyed on `h.HasAuth()`, which auth-by-default makes true on every run): anonymous callers get 401, signed-in non-superadmins 403, and only a session carrying the `platform_admin` role reaches the handler.
