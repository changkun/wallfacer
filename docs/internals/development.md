...
| `image` | Builds and pushes `ghcr.io/changkun/wallfacerd` (semver + sha tags) |
| `deploy` | Rolls the new image into the `latere` k8s namespace and smokes `wf.latere.ai` |
| `release` | Publishes the GitHub release with auto-generated notes and the binaries attached |
...
`-ldflags "-X latere.ai/x/wallfacer/internal/cli.Version=X.Y.Z -X ...SandboxTag=<images-tag>"`,
stamping the version for `wallfacer doctor` and usage output and pinning the
sandbox image tag.

**Release notes.** Generated by GitHub from the commit/PR history since the
previous tag (`gh release create --generate-notes`). Older releases are
archived under `docs/releases/`.
