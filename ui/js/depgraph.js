...
(function () {
  ...
  function getSubgraph(tasks) {
    ...
    for (var i = 0; i < tasks.length; i++) {
      ...
      if (t.depends_on && t.depends_on.length > 0) {
        involved.add(t.id);
        for (var j = 0; j < t.depends_on.length; j++) involved.add(t.depends_on[j]);
      }
    }
    return tasks.filter(function (t) { return involved.has(t.id); });
  }
  ...
  function kahnLevels(subgraph) {
    ...
    while (queue.length > 0) {
      ...
      levels.push(queue.map(function (id) { return idToTask.get(id); }).filter(Boolean));
    ...
    var cycleNodes = subgraph.filter(function (t) { return !processed.has(t.id); });
  ...
  function computeLayout(levels, cycleNodes) {
    ...
    var svgW = totalCols > 0
      ? PAD + totalCols * NODE_W + Math.max(0, totalCols - 1) * H_GAP + PAD
      : PAD * 2;
    var svgH = maxY + PAD;

    return { positions: positions, svgW: svgW, svgH: svgH, hasCycles: cycleNodes.length > 0 };
  }
    ...
    positions.forEach(function (pos) {
      ...
      (function (tid) {
        g.addEventListener('click', function () {
          if (typeof window.openTaskModal === 'function') window.openTaskModal(tid);
        });
      })(t.id);
      ...
      var truncated = rawTitle.length > 22 ? rawTitle.slice(0, 22) + '\u2026' : rawTitle;
  ...
  function renderDependencyGraph(tasks) {
    ...
    tasks.forEach(function (t) {
      if (t.depends_on) t.depends_on.forEach(function (d) { targetIds.add(d); });
    });
    var relevant = tasks.filter(function (t) {
      return hasDepEdge(t) || targetIds.has(t.id);
    });
    var fingerprint = JSON.stringify(relevant.map(function (t) {
      return { id: t.id, status: t.status, depends_on: t.depends_on, position: t.position, title: t.title };
    }));
