...
async function loadSystemPrompts() {
  ...
  try {
    ...
    list.innerHTML = '<div style="font-size:11px;color:var(--text-muted);padding:6px;">Error loading templates: ' + escapeHtml(e.message) + '</div>';
    return;
  }
  ...
  _systemPromptsData.forEach(function(tmpl) {
    ...
    btn.style.cssText = 'display:flex;align-items:center;gap:6px;width:100%;text-align:left;padding:5px 8px;border:1px solid transparent;border-radius:5px;background:none;cursor:pointer;font-size:12px;color:var(--text-secondary);';
    btn.onmouseover = function() { if (tmpl.name !== _systemPromptCurrent) btn.style.background = 'var(--bg-hover,rgba(128,128,128,0.08))'; };
    btn.onmouseout  = function() { if (tmpl.name !== _systemPromptCurrent) btn.style.background = 'none'; };

    // Dot indicator for user override.
    var dot = document.createElement('span');
    dot.style.cssText = 'width:6px;height:6px;border-radius:50%;flex-shrink:0;background:' + (tmpl.has_override ? 'var(--accent,#d97757)' : 'transparent') + ';border:1px solid ' + (tmpl.has_override ? 'var(--accent,#d97757)' : 'var(--border,#ccc)') + ';';
    dot.title = tmpl.has_override ? 'User override active' : 'Using embedded default';
    btn.appendChild(dot);
    ...
    btn.onclick = function() { selectSystemPrompt(tmpl.name); };
    list.appendChild(btn);
  });

  // Re-select the previously selected template if it still exists.
  if (_systemPromptCurrent && _systemPromptsData.some(function(t) { return t.name === _systemPromptCurrent; })) {
...
function selectSystemPrompt(name) {
  ...
  if (list) {
    Array.from(list.querySelectorAll('button')).forEach(function(btn) {
      ...
      btn.style.background = isSelected ? 'var(--bg-active,rgba(128,128,128,0.15))' : 'none';
      btn.style.borderColor = isSelected ? 'var(--border)' : 'transparent';
    });
  }

  var tmpl = _systemPromptsData.find(function(t) { return t.name === name; });
  if (!tmpl) return;

  var label = document.getElementById('system-prompt-name-label');
  if (label) label.textContent = name + (tmpl.has_override ? ' (override active)' : ' (embedded default)');
...
async function saveSystemPrompt() {
  ...
  try {
    await api('/api/system-prompts/' + encodeURIComponent(_systemPromptCurrent), {
      method: 'PUT',
...
async function resetSystemPromptToDefault() {
  if (!_systemPromptCurrent) return;
  var tmpl = _systemPromptsData.find(function(t) { return t.name === _systemPromptCurrent; });
  if (!tmpl || !tmpl.has_override) return;
  if (!confirm('Reset "' + _systemPromptCurrent + '" to the embedded default?\nYour override will be deleted.')) return;

  var statusEl = document.getElementById('system-prompt-status');
  if (statusEl) { statusEl.textContent = 'Resetting\u2026'; statusEl.style.color = 'var(--text-muted)'; }
  try {
    await api('/api/system-prompts/' + encodeURIComponent(_systemPromptCurrent), { method: 'DELETE' });
    if (statusEl) { statusEl.textContent = 'Reset to default.'; statusEl.style.color = 'var(--text-muted)'; }
    setTimeout(function() { if (statusEl) statusEl.textContent = ''; }, 2000);
    await loadSystemPrompts();
  } catch (e) {
    if (statusEl) { statusEl.textContent = 'Error: ' + e.message; statusEl.style.color = 'var(--color-error,#e53e3e)'; }
  }
}
