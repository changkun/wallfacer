...
if (typeof globalThis === "object") {
  Object.defineProperty(globalThis, "_commandPaletteServerSeq", {
    configurable: true,
    get: function () {
      return _commandPaletteServerSeq;
    },
    set: function (value) {
      _commandPaletteServerSeq = value;
    },
  });
}
...
function _hasWorktree(task) {
  return !!(
    task &&
    task.worktree_paths &&
    Object.keys(task.worktree_paths).length > 0
  );
}
...
function _setActiveRow(index) {
  ...
  const els = _getPaletteElements();
  const rowEls =
    els.results && typeof els.results.querySelectorAll === "function"
...
function _updateContextActionsForActiveRow(taskRows) {
  ...
  const selectedTask =
    _resolveTaskById(active.id) || (selectedTaskRow && selectedTaskRow.taskObj);
...
function commandPaletteMoveDown() {
  moveCommandPaletteActiveRow(1);
}
function commandPaletteMoveUp() {
  moveCommandPaletteActiveRow(-1);
}
...
function _buildTaskListSections(groups) {
    ...
    for (const row of group.rows) {
      ...
      if (row.type === "task") {
        ...
        const status = row.status
          ? `<span class="command-palette-task-badge">${row.status}</span>`
          : "";
        const idLabel = row.idHint
          ? `<span class="command-palette-task-id">${row.idHint}</span>`
          : "";
        const snippet = row.snippet
          ? `<div class="command-palette-task-snippet">${escapeHtml(row.snippet)}</div>`
          : "";
        const trimmedPrompt = row.prompt ? row.prompt.slice(0, 180) : "";
        const promptSnippet =
          !row.snippet && trimmedPrompt
            ? '<div class="command-palette-task-snippet">' +
              escapeHtml(trimmedPrompt) +
              "</div>"
            : "";
        rowEl.innerHTML =
        ...
        rowEl.innerHTML =
          ...
          "</div>" +
          (row.hint
            ? '<div class="command-palette-row-hint">' +
              escapeHtml(row.hint) +
              "</div>"
            : "");
      }
  ...
  if (
    ...
    _commandPaletteActiveIndex >= _commandPaletteRows.length
  ) {
    _setActiveRow(0);
  } else {
    _setActiveRow(_commandPaletteActiveIndex);
