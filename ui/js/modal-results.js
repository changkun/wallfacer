...
function _humanSpanLabel(phase, label) {
  ...
  if (phase === "agent_turn") {
    if ((m = label.match(/^implementation_(\d+)$/)))
      return "Implementation Turn " + m[1];
  ...
  if (phase === "board_context") {
    var bc;
    if ((bc = label.match(/^board_context_(\d+)$/)))
      return "Board Sync (Turn " + bc[1] + ")";
    return "Board Sync";
  }
...
function _buildTimelineHtml(spans) {
  ...
  for (var ni = 0; ni < niceIntervals.length; ni++) {
    if (totalMs / niceIntervals[ni] <= 8) {
      TICK_MS = niceIntervals[ni];
      break;
    }
  }
  ...
  if (timeMap.compressed) {
    ...
    tickFracs.forEach(function (f) {
      ...
      ticksHtml +=
        ...
        'font-size:10px;color:var(--text-muted);white-space:nowrap;user-select:none;">' +
        lbl +
        "</div>" +
        '<div style="position:absolute;left:' +
        x.toFixed(2) +
  ...
  spans.forEach(function (span) {
    ...
    rowsHtml +=
      ...
      '<div style="width:' +
      LABEL_W +
      "px;flex-shrink:0;font-size:11px;color:var(--text-muted);" +
      "overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" +
      'padding-right:6px;text-align:right;" title="' +
      escapeHtml(span.phase + ":" + span.label) +
      '">' +
  ...
  return (
    ...
    (gapLabelOverlay
      ? '<div style="position:absolute;top:0;left:' +
        LABEL_W +
        'px;right:0;bottom:0;pointer-events:none;">' +
        gapLabelOverlay +
        "</div>"
      : "") +
    "</div>" +
...
function _startTimelineRefresh(taskId) {
  ...
  if (
    ...
    typeof timelineRefreshTimer === "undefined"
  )
    return;
  var task = tasks.find(function (t) {
    return t.id === taskId;
  });
  if (!task || (task.status !== "in_progress" && task.status !== "committing"))
    return;
...
function renderTimeline(taskId) {
  ...
  var req =
    typeof apiGet === "function"
      ...
      : fetch("/api/tasks/" + taskId + "/spans", { signal: signal }).then(
          function (res) {
            return res.json();
          },
        );
  req
    ...
    .catch(function (err) {
      ...
      if (el2)
        el2.innerHTML =
          '<div style="font-size:12px;color:var(--text-muted);padding:8px 0;">Failed to load timeline.</div>';
    });
}
