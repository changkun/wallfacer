...
function _pullPhaseText(phase, layersDone) {
  switch (phase) {
    ...
    case "copying":
      return "Downloading layers (" + layersDone + " copied)\u2026";
    case "manifest":
...
async function loadImageStatus() {
  ...
  try {
    ...
    data.images.forEach(function (img) {
      ...
      row.style.cssText =
        "display:flex; align-items:center; gap:8px; padding:8px 10px; border:1px solid var(--border); border-radius:6px; font-size:12px;";

      var badge;
      if (isPulling) {
        badge =
          '<span style="display:inline-block;padding:1px 6px;border-radius:4px;background:#fef9c3;color:#854d0e;font-size:11px;font-weight:600;">Pulling\u2026</span>';
      } else if (img.cached) {
        badge =
      ...
      var info =
        ...
        escapeHtml(sb) +
        "</div>" +
        '<div style="font-size:11px;color:var(--text-muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="' +
        escapeHtml(img.image) +
        '">' +
        escapeHtml(img.image) +
        "</div>" +
        (img.size
          ? '<div style="font-size:11px;color:var(--text-muted);">Size: ' +
            escapeHtml(img.size) +
            "</div>"
          : "") +
        "</div>";

      var btnLabel = isPulling
        ? "Pulling\u2026"
        : img.cached
          ? "Re-pull"
          : "Pull";
      ...
      if (img.cached && !isPulling) {
        deleteBtn =
          ' <button type="button" class="btn-icon" style="font-size:12px;padding:4px 10px;white-space:nowrap;color:var(--text-error,#dc2626);" onclick="deleteSandboxImage(\'' +
          escapeHtml(sb) +
          "')\">Delete</button>";
      }
      ...
      row.innerHTML =
        ...
        ' <button id="' +
        btnId +
        '" type="button" class="btn-icon" style="font-size:12px;padding:4px 10px;white-space:nowrap;"' +
        (isPulling ? " disabled" : "") +
        " onclick=\"pullSandboxImage('" +
        escapeHtml(sb) +
        "')\">" +
        btnLabel +
        "</button>" +
      ...
      summaryEl.style.cssText =
        (isPulling ? "display:block;" : "display:none;") +
      ...
      if (
        active &&
        active.pullId &&
        (!active.source || active.source.readyState === EventSource.CLOSED)
      ) {
    ...
    container.innerHTML =
      '<div style="font-size:12px; color:var(--text-error,red);">Failed to load image status.</div>';
  ...
  source.addEventListener("progress", function (e) {
    try {
      ...
      if (sumEl && data.phase) {
        ...
        if (_activePulls[sandboxType])
          _activePulls[sandboxType].lastPhaseText = phaseText;
      }
    } catch (_) {}
  });
