...
function getOpenModalTaskId() {
  return _modalState.taskId;
}
...
function _renderModalLoadingPlaceholders() {
  ...
  if (eventsEl)
    eventsEl.innerHTML =
      '<span class="text-xs text-v-muted">Loading events…</span>';

  const diffEl = document.getElementById("modal-diff-files");
  if (diffEl)
    diffEl.innerHTML =
      '<span class="text-xs text-v-muted">Loading diff…</span>';
  ...
  if (logsEl)
    logsEl.innerHTML =
      '<div class="oversight-loading">Loading oversight…</div>';
  const testLogsEl = document.getElementById("modal-test-logs");
  if (testLogsEl)
    testLogsEl.innerHTML =
      '<div class="oversight-loading">Loading oversight…</div>';

  const timelineEl = document.getElementById("modal-timeline-chart");
  if (timelineEl) {
    timelineEl.innerHTML =
      '<div style="font-size:12px;color:var(--text-muted);padding:8px 0;">Loading timeline…</div>';
    delete timelineEl.dataset.loaded;
  }
}

function _modalDependencyIds(task) {
  if (typeof getTaskDependencyIds === "function")
...
function renderModalDependencies(task) {
  ...
  var listEl = document.getElementById("modal-dependencies-list");
  var summaryEl = document.getElementById("modal-dependencies-summary");
  if (!listEl) return;

  var nonDoneCount = 0;
  var rows = deps.map(function (depId) {
    var depTask =
      typeof findTaskById === "function" ? findTaskById(depId) : null;
    if (!depTask) {
      nonDoneCount++;
      return (
        '<div class="flex items-center gap-2 text-sm text-v-muted">' +
        '<span style="text-decoration:line-through;opacity:0.75;">[removed] ' +
        escapeHtml(depId.slice(0, 8)) +
        "</span>" +
        "</div>"
      );
    }
    ...
    var statusLabel =
      depTask.status === "in_progress" ? "in progress" : depTask.status;
    var label = _modalDependencyLabel(depTask, depId);
    return (
      '<div class="flex items-center gap-2 text-sm" data-dep-id="' +
      escapeHtml(depId) +
      '">' +
      '<span class="badge ' +
      escapeHtml(_modalDependencyStatusClass(depTask.status)) +
      '">' +
...
async function openModal(id) {
  function isTestCard(task) {
    return (
      task.status === "waiting" &&
      !!task.last_test_result &&
      task.test_run_start_turn > 0
    );
  }
  ...
  modalEl._onBackdropClick = function (e) {
  ...
  document.getElementById("modal-time").textContent = new Date(
    task.created_at,
  ).toLocaleString();
  ...
  if (task.status === "backlog") {
    ...
    // Populate settings
    document.getElementById("modal-edit-timeout").value = String(
      task.timeout || 60,
    );
    document.getElementById("modal-edit-mount-worktrees").checked =
      !!task.mount_worktrees;
    document.getElementById("modal-edit-sandbox").value = task.sandbox || "";
    const editModelOverrideEl = document.getElementById(
      "modal-edit-model-override",
    );
    if (editModelOverrideEl)
      editModelOverrideEl.value = task.model_override || "";
    const editMaxCostEl = document.getElementById("modal-edit-max-cost-usd");
    if (editMaxCostEl)
      editMaxCostEl.value =
        task.max_cost_usd > 0 ? String(task.max_cost_usd) : "";
    const editMaxTokensEl = document.getElementById(
    ...
    const editScheduledAtEl = document.getElementById(
  ...
  if (u && (u.input_tokens || u.output_tokens || u.cost_usd)) {
    document.getElementById("modal-usage-input").textContent =
      u.input_tokens.toLocaleString();
    document.getElementById("modal-usage-output").textContent =
      u.output_tokens.toLocaleString();
    document.getElementById("modal-usage-cache-read").textContent =
      u.cache_read_input_tokens.toLocaleString();
    document.getElementById("modal-usage-cache-creation").textContent =
      u.cache_creation_input_tokens.toLocaleString();
    document.getElementById("modal-usage-cost").textContent =
    ...
    if (
      budgetWrap &&
      budgetEl &&
      (task.max_cost_usd > 0 || task.max_input_tokens > 0)
    ) {
      const parts = [];
      if (task.max_cost_usd > 0) {
        parts.push(
    ...
    if (bd && Object.keys(bd).length > 0) {
      ...
      keys.forEach(function (agent) {
        ...
        row.style.cssText =
          "display:flex;justify-content:space-between;align-items:baseline;font-size:12px;padding:2px 0;";
        ...
        if (au.input_tokens || au.output_tokens) {
          parts.push(
            (au.input_tokens || 0).toLocaleString() +
              " in / " +
              (au.output_tokens || 0).toLocaleString() +
              " out",
          );
        }
        if (au.cost_usd) {
    ...
    if (envData) {
      ...
      if (envList) {
        function _envRow(label, valueHtml) {
          return (
            '<dt style="color:var(--text-muted);white-space:nowrap;">' +
            label +
            "</dt>" +
        ...
        const digestShort = envData.container_digest
          ? escapeHtml(envData.container_digest.slice(0, 12))
          : "";
        const containerVal = envData.container_image
          ? escapeHtml(envData.container_image) +
            (digestShort
              ? ' <code style="font-size:10px;font-family:monospace;">' +
                digestShort +
                "</code>"
              : "")
          : '<em style="color:var(--text-muted);">(unknown)</em>';
        const instrShort = envData.instructions_hash
          ? envData.instructions_hash.slice(0, 12)
          : "";
        const apiEndpoint = envData.api_base_url
          ? escapeHtml(envData.api_base_url)
          : '<em style="color:var(--text-muted);">(default)</em>';
        const recordedStr = envData.recorded_at
          ? escapeHtml(timeAgo(envData.recorded_at))
          : "";
        const sandboxVal = envData.sandbox
          ? escapeHtml(envData.sandbox)
          : '<em style="color:var(--text-muted);">(default)</em>';
        const rows = [
  ...
  if (task.status !== "backlog") {
    ...
    if (hasWorktrees) {
      ...
      if (commitMsgEl) {
        if (task.commit_message) {
          commitMsgEl.innerHTML =
            ...
            escapeHtml(task.commit_message) +
            "</pre>";
      ...
      api(`/api/tasks/${task.id}/diff`, { signal: modalLoad.signal })
        .then((data) => {
            ...
            if (!isTestCard(task) && totalBehind > 0) {
              const label =
                entries.length === 1
                  ...
                  : entries.map(([repo, n]) => `${repo}: ${n}`).join(", ") +
                    " behind";
              warnEl.innerHTML =
  ...
  if (task.prompt_history && task.prompt_history.length > 0) {
    ...
    historyList.innerHTML = task.prompt_history
      .map(
        (p, i) =>
  ...
  // Retry history
  const retryHistorySection = document.getElementById(
    "modal-retry-history-section",
  );
  if (task.retry_history && task.retry_history.length > 0) {
    retryHistorySection.classList.remove("hidden");
    const retryHistoryList = document.getElementById(
      "modal-retry-history-list",
    );
    retryHistoryList.innerHTML = task.retry_history
      .map(function (rec, i) {
        const attemptNum = i + 1;
        const retiredAgo = timeAgo(rec.retired_at);
        const costStr = rec.cost_usd
          ? "$" + rec.cost_usd.toFixed(4)
          : "$0.0000";
        const detailId = "retry-history-detail-" + i;
        const promptSnippet = rec.prompt
          ? escapeHtml(rec.prompt)
          : '<em class="text-v-muted">no prompt</em>';
        const resultSnippet = rec.result
          ? escapeHtml(rec.result)
          : '<em class="text-v-muted">no result</em>';
        return (
          '<div style="border:1px solid var(--border);border-radius:6px;padding:8px 10px;font-size:12px;">' +
          '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">' +
          '<span style="font-weight:600;color:var(--text-muted);font-size:11px;">Attempt #' +
          attemptNum +
          "</span>" +
          ...
          escapeHtml(rec.retired_at) +
          '">' +
          escapeHtml(retiredAgo) +
          "</span>" +
          ...
          detailId +
          '\').classList.toggle(\'hidden\')" class="btn-icon" style="margin-left:auto;">Show</button>' +
          "</div>" +
          '<div id="' +
          detailId +
          '" class="hidden" style="margin-top:8px;border-top:1px solid var(--border);padding-top:8px;">' +
          '<div style="margin-bottom:6px;">' +
          '<span style="font-size:10px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.05em;">Prompt</span>' +
          '<pre class="code-block text-xs" style="margin-top:4px;white-space:pre-wrap;">' +
          promptSnippet +
          "</pre>" +
          "</div>" +
          (rec.result
  ...
  function _renderEventRow(e) {
    ...
    if (data.phase === "conflict_resolver") {
      const trigger = data.trigger
        ? ` · ${escapeHtml(String(data.trigger))}`
        : "";
      ...
      const badgeStyle =
        statusColors[data.status] || "background:#6b7280;color:#fff";
      ...
      detail =
        `<div style="border-left:3px solid #f59e0b;padding:8px 10px;margin:4px 0;">` +
      ...
      if (
        data.phase === "rebase" &&
        Array.isArray(data.conflicted_files) &&
        data.conflicted_files.length > 0
      ) {
        detail =
          '<div style="border-left:3px solid #ef4444;padding:8px 10px;margin:4px 0;">' +
  ...
  try {
    ...
    const implResults =
      testStartTurn > 0 ? outputResults.slice(0, testStartTurn) : outputResults;
    const testResults =
      testStartTurn > 0 ? outputResults.slice(testStartTurn) : [];
    ...
    const budgetBanner = document.getElementById(
    ...
    if (budgetBanner) {
      ...
      const isBudgetExceeded =
        ...
        lastSystem &&
        lastSystem.data &&
        lastSystem.data.budget_exceeded;
      if (isBudgetExceeded) {
        ...
        if (bannerMsg)
          bannerMsg.textContent =
            lastSystem.data.message || "Budget limit reached";
      } else {
    ...
    document.getElementById("modal-events").innerHTML =
      '<span class="text-xs ev-error">Failed to load events</span>';
  }
...
function closeModal() {
  ...
  if (_modalState.abort) {
    _modalState.abort.abort();
    _modalState.abort = null;
  }
  _modalState.seq += 1;
  ...
  const canRestoreFocus =
    _previousModalFocus &&
    typeof _previousModalFocus.focus === "function" &&
