...
describe("summarizeSandboxTestResult", () => {
  it("normalizes pass/fail and status responses", () => {
    ...
    expect(ctx.summarizeSandboxTestResult({ last_test_result: "pass" })).toBe(
      "PASS",
    );
    expect(ctx.summarizeSandboxTestResult({ last_test_result: "FAIL" })).toBe(
      "FAIL",
    );
    expect(ctx.summarizeSandboxTestResult({ status: "done" })).toBe(
      "Test completed",
    );
    expect(
      ctx.summarizeSandboxTestResult({
        status: "failed",
        result: "Syntax error",
      }),
    ).toBe("Syntax error");
    expect(ctx.summarizeSandboxTestResult({ status: "running" })).toBe(
      "status running",
    );
  });
});
