...
describe('createTasksState', () => {
  ...
  it('clones the tasks and archivedTasks arrays (does not mutate the input)', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const input = { tasks: [task('a')], archivedTasks: [task('b', { archived: true })], archivedPage: {} };
...
describe('applyTasksSnapshot', () => {
  ...
  it('leaves archivedTasks untouched', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const state = { ...emptyState(), archivedTasks: [task('arch', { archived: true })] };
...
describe('applyTaskDeleted', () => {
  ...
  it('removes the identified task from the archived list', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const state = { ...emptyState(), archivedTasks: [task('arch-1', { archived: true }), task('arch-2', { archived: true })] };
  ...
  it('removes from both active and archived when the id appears in both', () => {
    ...
    const state = {
      ...
      archivedTasks: [task('t2', { archived: true }), task('t3', { archived: true })],
      archivedPage: { loadState: 'idle', hasMoreBefore: false, hasMoreAfter: false },
    };
...
describe('applyTaskUpdated', () => {
  it('moves an active task to archivedTasks when archived=true and showArchived=true', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const state = { ...emptyState(), tasks: [task('t1', { updated_at: '2026-03-10T10:00:00Z' })] };
    const { state: next } = ctx.applyTaskUpdated(state, task('t1', { archived: true, updated_at: '2026-03-10T11:00:00Z' }), { showArchived: true, pageSize: 20 });
    expect(next.tasks).toEqual([]);
  ...
  it('discards an archived task from state when showArchived=false', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const state = { ...emptyState(), tasks: [task('t1')] };
    const { state: next } = ctx.applyTaskUpdated(state, task('t1', { archived: true }), { showArchived: false });
    expect(next.tasks).toEqual([]);
  ...
  it('restores an unarchived task from archivedTasks back to active tasks', () => {
    ...
    const state = {
      tasks: [],
      archivedTasks: [task('t1', { archived: true, updated_at: '2026-03-10T11:00:00Z' })],
      archivedPage: { loadState: 'idle', hasMoreBefore: false, hasMoreAfter: false },
    };
    const { state: next } = ctx.applyTaskUpdated(state, task('t1', { archived: false, status: 'done', updated_at: '2026-03-10T12:00:00Z' }), { showArchived: true, pageSize: 20 });
    expect(next.tasks.map((t) => t.id)).toEqual(['t1']);
    expect(next.archivedTasks).toEqual([]);
  });
  ...
  it('returns the previousTask record for status-change announcements', () => {
    ...
    const { previousTask } = ctx.applyTaskUpdated(state, task('t1', { status: 'in_progress' }), {});
  ...
  it('returns null previousTask for a brand-new task', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const { previousTask } = ctx.applyTaskUpdated(emptyState(), task('brand-new'), {});
...
describe('mergeArchivedTasksPage', () => {
  it('replaces archived window on initial load', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const state = { ...emptyState(), archivedTasks: [task('old', { archived: true })] };
    const next = ctx.mergeArchivedTasksPage(state, { tasks: [task('new', { archived: true })], has_more_before: false, has_more_after: false }, 'initial', 20);
    expect(next.archivedTasks.map((t) => t.id)).toEqual(['new']);
  });

  it('appends unique tasks when loading the next page (direction=before)', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const existing = task('b', { archived: true, updated_at: '2026-03-10T10:00:00Z' });
    const incoming = task('c', { archived: true, updated_at: '2026-03-10T09:00:00Z' });
    const state = { ...emptyState(), archivedTasks: [existing] };
    const next = ctx.mergeArchivedTasksPage(state, { tasks: [incoming], has_more_before: true, has_more_after: false }, 'before', 20);
  ...
  it('sets pagination flags from the response', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const next = ctx.mergeArchivedTasksPage(emptyState(), { tasks: [], has_more_before: true, has_more_after: true }, 'initial', 20);
...
describe('trimArchivedWindowState', () => {
  it('does not trim when count is within pageSize * 3', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const archivedTasks = Array.from({ length: 6 }, (_, i) => task(`t${i}`, { archived: true, updated_at: `2026-03-10T0${6 - i}:00:00Z` }));
  ...
  it('trims the oldest entries (tail) when loading after and flags hasMoreBefore', () => {
    ...
    const archivedTasks = Array.from({ length: 7 }, (_, i) =>
      task(`task-${i}`, { archived: true, updated_at: `2026-03-10T0${6 - i}:00:00Z` }),
    );
    const state = { ...emptyState(), archivedTasks };
    const next = ctx.trimArchivedWindowState(state, 'after', 2); // maxItems = 6, keep first 6
    expect(next.archivedTasks.map((t) => t.id)).toEqual(['task-0', 'task-1', 'task-2', 'task-3', 'task-4', 'task-5']);
    expect(next.archivedPage.hasMoreBefore).toBe(true);
  ...
  it('trims the newest entries (head) when loading before and flags hasMoreAfter', () => {
    ...
    expect(next.archivedTasks.map((t) => t.id)).toEqual(['task-1', 'task-2', 'task-3', 'task-4', 'task-5', 'task-6']);
    expect(next.archivedPage.hasMoreAfter).toBe(true);
...
describe('buildTasksStreamUrl', () => {
  it('returns the base URL unchanged when no event ID is provided', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    expect(ctx.buildTasksStreamUrl('/api/tasks/stream', null)).toBe('/api/tasks/stream');
    expect(ctx.buildTasksStreamUrl('/api/tasks/stream', undefined)).toBe('/api/tasks/stream');
  });
  ...
  it('uses & instead of ? when the base URL already has query parameters', () => {
    const ctx = loadScript(makeContext(), 'task-stream.js');
    const url = ctx.buildTasksStreamUrl('/api/tasks/stream?token=abc', 'evt-42');
  ...
  it('delegates token injection to withAuthToken (stub passes url through)', () => {
    ...
    let called = false;
    const ctx = makeContext({ withAuthToken: (url) => { called = true; return url; } });
