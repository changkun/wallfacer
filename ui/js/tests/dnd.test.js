...
describe("initSortable", () => {
  ...
  it("blocks refinement transitions on add and updates running refinements only", () => {
    ...
    expect(backlog.insertBefore).toHaveBeenCalledWith(
      { dataset: { id: "task-ref" } },
      backlog.children[0],
    );
    expect(ctx.showAlert).toHaveBeenCalledWith(
      "Refinement is in progress. Please wait for it to complete before starting.",
    );

    inProgressCfg.onAdd({
      item: { dataset: { id: "task-done" } },
      oldIndex: 1,
    });
    expect(ctx.showAlert).toHaveBeenCalledWith(
      "This task has a refined prompt awaiting review. Open the task to apply or dismiss it before starting.",
    );
