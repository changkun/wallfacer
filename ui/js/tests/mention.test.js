...
function createElement(overrides = {}) {
  ...
  const node = {
    ...
    getBoundingClientRect() {
      return { bottom: 5, left: 2, width: 100 };
    },
...
describe("_mentionFilter", () => {
  it("returns a file-order-limited top match set", () => {
    ...
    const files = Array.from(
      { length: 25 },
      (_, idx) => `files/${String(idx).padStart(2, "0")}-notes.txt`,
    );
...
describe("_mentionLoadFiles", () => {
  it("caches successful responses and returns [] while loading", async () => {
    let resolve;
    const loadingPromise = new Promise((resolveFn) => {
      resolve = resolveFn;
    });
    const api = vi
      .fn()
      .mockImplementation(() =>
        loadingPromise.then(() => ({ files: ["a.go", "b.go"] })),
      );
...
describe("attachMentionAutocomplete", () => {
  ...
  it("auto-selects first match so Enter/Tab completes without arrow navigation", async () => {
    ...
    const dropdown = ctx.document.body._children.find((n) =>
    ...
    const selectedItems = dropdown._children.filter(
      (item) =>
        item.className && item.className.includes("mention-item-selected"),
    );
