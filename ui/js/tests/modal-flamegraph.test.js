  ...
  it("returns a value in [0, 360)", () => {
    ...
    inputs.forEach((s) => {
      const h = vm.runInContext(
        `_flamegraph.labelHue(${JSON.stringify(s)})`,
        ctx,
      );
...
describe("loadFlamegraph", () => {
  ...
  it("renders span blocks for non-empty response", async () => {
    ...
    const { ctx, container } = makeFlameContext((url) => {
...
describe("loadFlamegraph oversight integration", () => {
  ...
  function makeDispatchFetch(spansResp, oversightResp) {
    return (url) => {
      ...
      return Promise.resolve({
        json: () => Promise.resolve({ spans: spansResp }),
      });
    };
  }
  ...
  it("renders normally when oversight fetch rejects", async () => {
    const fetch = (url) => {
      ...
      return Promise.resolve({
        json: () => Promise.resolve({ spans: spansFixture }),
      });
    };
...
describe("spanActivity", () => {
  ...
  it('returns "implementation" for agent_turn:implementation_N', () => {
    const result = vm.runInContext(
      '_flamegraph.spanActivity("agent_turn:implementation_3")',
      ctx,
    );
  ...
  it('returns "testing" for agent_turn:test_N', () => {
    const result = vm.runInContext(
      '_flamegraph.spanActivity("agent_turn:test_2")',
      ctx,
    );
  ...
  it('returns "implementation" for legacy agent_turn:agent_turn_N', () => {
    const result = vm.runInContext(
      '_flamegraph.spanActivity("agent_turn:agent_turn_1")',
      ctx,
    );
    expect(result).toBe("implementation");
  });
  ...
  it("returns the label for container_run:* spans", () => {
    ...
    activities.forEach((act) => {
      const result = vm.runInContext(
        `_flamegraph.spanActivity("container_run:${act}")`,
        ctx,
      );
  ...
  it('returns "refinement" for refinement phase', () => {
    const result = vm.runInContext(
      '_flamegraph.spanActivity("refinement:refinement")',
      ctx,
    );
  ...
  it("returns empty string for worktree_setup", () => {
    const result = vm.runInContext(
      '_flamegraph.spanActivity("worktree_setup:worktree_setup")',
      ctx,
    );
    expect(result).toBe("");
  });
  ...
  it("returns empty string for commit phase", () => {
    const result = vm.runInContext(
      '_flamegraph.spanActivity("commit:commit")',
      ctx,
    );
    expect(result).toBe("");
  });
  ...
  it("has entries for all SandboxActivity constants", () => {
    ...
    required.forEach((key) => {
      const label = vm.runInContext(
        `_flamegraph.ACTIVITY_LABELS[${JSON.stringify(key)}]`,
        ctx,
      );
