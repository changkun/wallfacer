...
describe("matchesFilter", () => {
  ...
  it("(a) hides non-matching cards — returns false when neither title nor prompt matches", () => {
    ...
    const task = {
      title: "Fix login bug",
      prompt: "The login form breaks on Safari.",
    };
  ...
  it("(b) is case-insensitive when matching titles", () => {
    setFilter(ctx, "hello");
    expect(
      ctx.matchesFilter({ title: "Hello World", prompt: "unrelated" }),
    ).toBe(true);
    expect(ctx.matchesFilter({ title: "HELLO", prompt: "unrelated" })).toBe(
      true,
    );
    expect(ctx.matchesFilter({ title: "say hello", prompt: "unrelated" })).toBe(
      true,
    );
  });
  ...
  it("(b) is case-insensitive when matching prompts", () => {
    ...
    expect(
      ctx.matchesFilter({ title: "Task", prompt: "implement the feature" }),
    ).toBe(true);
    expect(ctx.matchesFilter({ title: "Task", prompt: "Implement now" })).toBe(
      true,
    );
  });
  ...
  it("(d) matches on title field", () => {
    setFilter(ctx, "auth");
    expect(
      ctx.matchesFilter({ title: "Add authentication", prompt: "unrelated" }),
    ).toBe(true);
    expect(ctx.matchesFilter({ title: "unrelated", prompt: "unrelated" })).toBe(
      false,
    );
  });
  ...
  it("(d) matches on prompt field", () => {
    ...
    expect(
      ctx.matchesFilter({ title: "UI task", prompt: "Light theme only" }),
    ).toBe(false);
  });
  ...
  it("handles tasks with missing title gracefully", () => {
    ...
    expect(ctx.matchesFilter({ title: null, prompt: "hello world" })).toBe(
      true,
    );
    expect(ctx.matchesFilter({ title: undefined, prompt: "no match" })).toBe(
      false,
    );
  });
  ...
  it("handles tasks with no tags gracefully", () => {
    ...
    expect(
      ctx.matchesFilter({ title: "unrelated", prompt: "unrelated", tags: [] }),
    ).toBe(false);
  });
  ...
  it("#tag token filters by exact tag match", () => {
    setFilter(ctx, "#backend");
    expect(
      ctx.matchesFilter({ title: "x", prompt: "y", tags: ["backend"] }),
    ).toBe(true);
    expect(
      ctx.matchesFilter({ title: "x", prompt: "y", tags: ["frontend"] }),
    ).toBe(false);
  ...
  it("multiple #tag tokens are AND-combined", () => {
    ...
    expect(
      ctx.matchesFilter({ title: "x", prompt: "y", tags: ["backend", "api"] }),
    ).toBe(true);
    expect(
      ctx.matchesFilter({ title: "x", prompt: "y", tags: ["backend"] }),
    ).toBe(false);
    expect(ctx.matchesFilter({ title: "x", prompt: "y", tags: ["api"] })).toBe(
      false,
    );
  });
  ...
  it("#tag + free text both must match", () => {
    ...
    expect(
      ctx.matchesFilter({ title: "unrelated", prompt: "y", tags: ["backend"] }),
    ).toBe(false);
  });
});
