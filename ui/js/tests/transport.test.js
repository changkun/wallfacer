...
function makeTokenDoc(token) {
  return {
    getElementById: () => null,
    querySelectorAll: () => [],
    querySelector: (sel) => sel === 'meta[name="wallfacer-token"]' ? { content: token } : null,
    addEventListener: () => {},
    documentElement: { setAttribute: () => {} },
    readyState: 'complete',
  };
...
describe('getWallfacerToken', () => {
  ...
  it('returns the token content from the meta tag', () => {
    const ctx = loadScript(makeContext({ document: makeTokenDoc('my-token') }), 'transport.js');
...
describe('withAuthHeaders', () => {
  it('does not add Authorization for GET requests even when a token is present', () => {
    const ctx = loadScript(makeContext({ document: makeTokenDoc('tok') }), 'transport.js');
    const headers = ctx.withAuthHeaders({}, 'GET');
    expect(headers.Authorization).toBeUndefined();
  });

  it('adds Bearer Authorization for POST requests when a token is present', () => {
    const ctx = loadScript(makeContext({ document: makeTokenDoc('secret') }), 'transport.js');
...
describe('withBearerHeaders', () => {
  it('adds Authorization unconditionally (used for streaming GET)', () => {
    const ctx = loadScript(makeContext({ document: makeTokenDoc('bearer-tok') }), 'transport.js');
...
describe('withAuthToken', () => {
  it('appends token as the first query parameter when URL has no query string', () => {
    const ctx = loadScript(makeContext({ document: makeTokenDoc('tok123') }), 'transport.js');
    expect(ctx.withAuthToken('/api/tasks/stream')).toBe('/api/tasks/stream?token=tok123');
  });

  it('appends token with & when URL already has query parameters', () => {
    const ctx = loadScript(makeContext({ document: makeTokenDoc('tok123') }), 'transport.js');
    expect(ctx.withAuthToken('/api/tasks/stream?foo=bar')).toBe('/api/tasks/stream?foo=bar&token=tok123');
  });
  ...
  it('URL-encodes the token', () => {
    const ctx = loadScript(makeContext({ document: makeTokenDoc('a b+c') }), 'transport.js');
...
describe('api()', () => {
  it('adds bearer auth to non-GET requests when a token meta tag is present', async () => {
    ...
    const ctx = loadScript(makeContext({ fetch, document: makeTokenDoc('secret-token') }), 'transport.js');
  ...
  it('does not add Authorization to GET requests', async () => {
    const fetch = vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => ({}), text: async () => '' });
    const ctx = loadScript(makeContext({ fetch, document: makeTokenDoc('secret') }), 'transport.js');
  ...
  it('throws on non-2xx responses', async () => {
    const fetch = vi.fn().mockResolvedValue({ ok: false, status: 500, text: async () => 'Internal Server Error' });
    const ctx = loadScript(makeContext({ fetch }), 'transport.js');

    await expect(ctx.api('/api/tasks')).rejects.toThrow('Internal Server Error');
  });
  ...
  it('parses and returns JSON on success', async () => {
    ...
    const fetch = vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => payload, text: async () => '' });
    const ctx = loadScript(makeContext({ fetch }), 'transport.js');
