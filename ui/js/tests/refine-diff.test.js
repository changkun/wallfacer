...
function makeRefineContext(overrides = {}) {
  ...
  elements["refine-history-section"] = makeEl("refine-history-section", [
    "hidden",
  ]);
...
describe("startRefinement — calls updateRefineUI with returned task", () => {
  it("shows running state immediately after POST returns 202", async () => {
    ...
    ctx._elements["refine-cancel-btn"] = makeEl("refine-cancel-btn", [
      "hidden",
    ]);
    ctx._elements["refine-result-section"] = makeEl("refine-result-section", [
      "hidden",
    ]);
    ctx._elements["refine-error-section"] = makeEl("refine-error-section", [
      "hidden",
    ]);
    ctx._elements["refine-idle-desc"] = makeEl("refine-idle-desc");
    ctx._elements["refine-instructions-section"] = makeEl(
      "refine-instructions-section",
    );
    ctx._elements["refine-user-instructions"] = {
      ...makeEl("refine-user-instructions"),
      value: "",
    };
    ctx._elements["refine-result-prompt"] = {
      ...makeEl("refine-result-prompt"),
      value: "",
    };
