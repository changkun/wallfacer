...
describe("extractToolInput", () => {
  ...
  it("extracts file_path from Read tool", () => {
    expect(ctx.extractToolInput("Read", { file_path: "/foo/bar.js" })).toBe(
      "/foo/bar.js",
    );
  });

  it("extracts file_path from Write tool", () => {
    expect(ctx.extractToolInput("Write", { file_path: "/out.txt" })).toBe(
      "/out.txt",
    );
  });

  it("extracts file_path from Edit tool", () => {
    expect(ctx.extractToolInput("Edit", { file_path: "/edit.go" })).toBe(
      "/edit.go",
    );
  });

  it("extracts pattern from Glob tool", () => {
    expect(ctx.extractToolInput("Glob", { pattern: "**/*.js" })).toBe(
      "**/*.js",
    );
  });

  it("extracts pattern from Grep tool", () => {
    expect(ctx.extractToolInput("Grep", { pattern: "function\\s+\\w+" })).toBe(
      "function\\s+\\w+",
    );
  });
  ...
  it("extracts url from WebFetch tool", () => {
    expect(
      ctx.extractToolInput("WebFetch", { url: "https://example.com" }),
    ).toBe("https://example.com");
  });

  it("extracts query from WebSearch tool", () => {
    expect(
      ctx.extractToolInput("WebSearch", { query: "vitest coverage" }),
    ).toBe("vitest coverage");
  });
  ...
  it("returns count for TodoWrite tool", () => {
    expect(ctx.extractToolInput("TodoWrite", { todos: [{}, {}, {}] })).toBe(
      "3 items",
    );
  });
  ...
  it("falls back to file_path for unknown tools with that key", () => {
    expect(
      ctx.extractToolInput("UnknownTool", { file_path: "/some/path" }),
    ).toBe("/some/path");
  });

  it("falls back to command for unknown tools with that key", () => {
    expect(
      ctx.extractToolInput("UnknownTool", { command: "do something" }),
    ).toBe("do something");
  });

  it("falls back to query for unknown tools with that key", () => {
    expect(ctx.extractToolInput("UnknownTool", { query: "search term" })).toBe(
      "search term",
    );
  });
...
describe("stripCodexShellWrapper", () => {
  ...
  it("strips /bin/bash -lc wrapper for single-quoted commands", () => {
    expect(ctx.stripCodexShellWrapper("/bin/bash -lc 'echo hello'")).toBe(
      "echo hello",
    );
  });
