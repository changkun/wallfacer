...
function createElement(tagName = "div", overrides = {}) {
  const element = {
    ...
    dispatchEvent(evt) {
      (this._listeners[evt.type] || []).forEach((handler) => {
        handler(evt);
      });
    },
    ...
    querySelectorAll(selector) {
      ...
      const matches = (node) => {
        ...
        if (selector === '[role="listitem"]')
          return node.attributes.role === "listitem";
...
describe("trash-bin", () => {
  ...
  it("renders deleted rows with title fallback and remaining days", async () => {
    const fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => [
        ...
        },
        {
          ...
          title: "",
          prompt:
            "This prompt should be truncated to sixty characters for the trash panel display and no more.",
          status: "backlog",
    ...
    expect(collectText(rows[1])).toContain(
      "This prompt should be truncated to sixty characters for the",
    );
    expect(collectText(rows[1])).toContain("\u2026");
  ...
  it("restores a deleted task, removes the row, and shows a toast", async () => {
    ...
    await ctx.restoreDeletedTask(
      "task-restore",
      row,
      restoreButton,
      "Bring back config migration",
    );

    expect(elements.get("trash-bin-list").children).toHaveLength(0);
    expect(elements.get("trash-bin-empty").classList.contains("hidden")).toBe(
      false,
    );
    expect(elements.get("trash-bin-toast").textContent).toBe(
      'Restored "Bring back config migration"',
    );
    expect(elements.get("trash-bin-toast").classList.contains("hidden")).toBe(
      false,
    );
