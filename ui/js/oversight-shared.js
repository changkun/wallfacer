...
function buildPhaseListHTML(phases) {
  ...
  return phases.map(function(phase, i) {
    var ts = phase.timestamp ? new Date(phase.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
    var tools = (phase.tools_used || []).map(function(t) {
      return '<span class="oversight-tool">' + escapeHtml(t) + '</span>';
    }).join('');
    var commands = (phase.commands || []).map(function(c) {
      return '<li class="oversight-command">' + escapeHtml(c) + '</li>';
    }).join('');
    var actions = (phase.actions || []).map(function(a) {
