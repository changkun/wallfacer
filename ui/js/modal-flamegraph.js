(function () {
  ...
  function humanSpanLabel(rawLabel) {
    ...
    var m;
    if (phase === "agent_turn") {
      if ((m = label.match(/^implementation_(\d+)$/)))
        return "Impl. Turn " + m[1];
  ...
  function buildCostChart(
      ...
      var xPct =
        ts !== null
          ? toPercentFn
            ? toPercentFn(ts)
            : total > 0
              ? Math.min(100, Math.max(0, ((ts - globalStartMs) / total) * 100))
              : null
          : null;
      if (xPct !== null) {
    ...
    return (
      ...
      chartH +
      'px;margin-top:4px;" ' +
      'title="Cumulative cost across all activities. Total: ' +
  ...
  function loadFlamegraph(taskId) {
    ...
    container.innerHTML =
      '<span style="color:var(--text-muted,#888);font-size:12px;">Loading\u2026</span>';
    ...
    Promise.all([
      fetchJson(spansUrl).catch(function () {
        return { spans: [] };
      }),
      fetchJson(oversightUrl).catch(function () {
        return null;
      }),
      fetchJson(turnUsageUrl).catch(function () {
        return [];
      }),
    ]).then(function (results) {
      if (getOpenModalTaskId() !== null && getOpenModalTaskId() !== taskId)
        return;
      if (_modalState.seq !== seq) return;
      var spansData = results[0];
      var records =
        spansData && Array.isArray(spansData.spans) ? spansData.spans : [];
      ...
      if (!records || records.length === 0) {
        container.innerHTML =
          '<span style="color:var(--text-muted,#888);font-size:12px;">No span data available.</span>';
        return;
      }
      ...
      assigned.forEach(function (a) {
        if (a.lane >= numLanes) numLanes = a.lane + 1;
      });
      ...
      if (
        oversightData &&
        oversightData.status === "ready" &&
        oversightData.phases &&
        oversightData.phases.length > 0
      ) {
        phaseRegions = computePhaseRegions(
          oversightData.phases,
          globalStartMs,
          globalEndMs,
        );
      ...
      if (timeMap.compressed) {
        ...
        axisHtml = tickFractions
          .map(function (f) {
            ...
            var transform =
              f === 0 ? "" : f === 1 ? "translateX(-100%)" : "translateX(-50%)";
            return (
              '<span style="position:absolute;left:' +
              pct +
              "%;font-size:10px;" +
              "color:var(--text-muted,#888);transform:" +
              transform +
              ";" +
              "text-align:" +
              align +
      ...
      var blocksHtml = assigned
        .map(function (a) {
          ...
          var tooltip =
            ...
            " (" +
            escapeHtml(span.rawLabel) +
            ")" +
            " | start: " +
            escapeHtml(startOffset) +
            " | dur: " +
            escapeHtml(formatMs(span.durationMs));
      ...
      var sortedByDuration = spans.slice().sort(function (a, b) {
        return b.durationMs - a.durationMs;
      });
      var rowsHtml = sortedByDuration
        .map(function (span) {
          var startOffset = formatMs(span.startMs - globalStartMs);
          var pct =
            total > 0 ? ((span.durationMs / total) * 100).toFixed(1) : "0.0";
          var hue = labelHue(span.rawLabel);
          var swatch =
            '<span style="display:inline-block;width:8px;height:8px;border-radius:2px;' +
            "background:hsl(" +
            hue +
            ',55%,52%);margin-right:4px;flex-shrink:0;"></span>';
          var rowPhaseMatch = findPhaseForSpan(span, phaseRegions);
          var oversightCell = rowPhaseMatch
            ? '<td style="padding:3px 6px;white-space:nowrap;max-width:120px;overflow:hidden;text-overflow:ellipsis;" title="' +
              escapeHtml(rowPhaseMatch.title) +
              '">' +
              escapeHtml(rowPhaseMatch.title) +
              "</td>"
            : '<td style="padding:3px 6px;color:var(--text-muted,#888);">&mdash;</td>';
          var activity = spanActivity(span.rawLabel);
          var activityDisplay = activity
          ...
          var activityCell = activityDisplay
            ? '<td style="padding:3px 6px;white-space:nowrap;">' +
              ...
              "background:hsl(" +
              activityHue +
              ',55%,52%);margin-right:4px;vertical-align:middle;"></span>' +
              escapeHtml(activityDisplay) +
              "</td>"
            : '<td style="padding:3px 6px;color:var(--text-muted,#888);">&mdash;</td>';
          return (
            '<tr style="border-bottom:1px solid var(--border,#333);" title="' +
            escapeHtml(span.rawLabel) +
            '">' +
            '<td style="padding:3px 6px;white-space:nowrap;">' +
            swatch +
            ...
            escapeHtml(formatMs(span.durationMs)) +
            "</td>" +
            '<td style="padding:3px 6px;text-align:right;white-space:nowrap;color:var(--text-muted,#888);">' +
            pct +
            "%</td>" +
            "</tr>"
          );
      ...
      var costChartHtml = buildCostChart(
        turnUsages,
        spans,
        globalStartMs,
        total,
