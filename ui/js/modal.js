...
function detectResultType(text) {
  ...
  return planPatterns.some(function (p) {
    return p.test(text);
  })
    ? "plan"
    : "result";
...
function renderResultsFromEvents(results, opts) {
  ...
  listEl.innerHTML = [...results]
    ...
    .map(function (result, i) {
      ...
      const turnLabelHtml =
        totalTurns > 1
          ? '<span class="result-turn-label">Turn ' +
            (originalIndex + 1) +
            "</span>"
          : "";
      const labelsHtml =
      ...
      const btnRowHtml =
        ...
        "<button onclick=\"copyResultEntry('" +
        entryId +
        '\')" class="btn-icon" title="Copy">' +
        _copyIcon +
        " Copy" +
        "</button>" +
        "<button onclick=\"toggleResultEntryRaw('" +
        entryId +
        '\')" class="btn-icon">Raw</button>' +
        "</div>";
      const bodyHtml =
        '<div id="' +
        entryId +
        '-rendered" class="result-entry-body prose-content">' +
        renderMarkdown(result) +
        "</div>" +
        '<pre id="' +
        entryId +
        '-raw" class="result-entry-body hidden">' +
        escapeHtml(result) +
        "</pre>";
...
function renderDiffLine(line) {
  ...
  if (line.startsWith("@@"))
    return `<span class="diff-line diff-hunk">${escaped}</span>`;
  if (/^(diff |--- |\+{3} |index |Binary )/.test(line))
    return `<span class="diff-line diff-header">${escaped}</span>`;
...
function renderDiffFiles(container, diff) {
  if (!diff) {
    container.innerHTML =
      '<span class="text-xs text-v-muted">No changes</span>';
    return;
  }
...
async function openModal(id) {
  ...
  if (task.status === "backlog") {
    ...
    const modalEditModelOverrideEl = document.getElementById(
      "modal-edit-model-override",
    );
    if (modalEditModelOverrideEl)
      modalEditModelOverrideEl.value = task.model_override || "";
  ...
  if (task.status !== "backlog") {
    ...
    if (
      (task.status === "failed" ||
        task.status === "done" ||
        (task.status === "waiting" && !isTestTaskCard)) &&
      hasWorktrees
    ) {
  ...
  try {
    ...
    const page = await api(
      `/api/tasks/${id}/events?limit=200&types=state_change,output,feedback,error,system`,
    );
    ...
    function _triggerBadge(trigger) {
      ...
      return style
        ? `<span style="font-size:10px;padding:1px 5px;border-radius:3px;${style};margin-left:4px;">${trigger}</span>`
        : "";
    }
    ...
    if (page.has_more) {
      ...
      loadMoreBtn.className =
        "text-xs text-v-muted hover:text-v-secondary mt-1";
      ...
      loadMoreBtn.onclick = async () => {
        ...
        try {
          const next = await api(
            `/api/tasks/${id}/events?limit=200&types=state_change,output,feedback,error,system&after=${cursor}`,
          );
          loadMoreBtn.remove();
    ...
    if (match[2] === "m") {
      ...
      while (i < codes.length) {
        ...
        if (c === 1) style += "font-weight:bold;";
          ...
          style += `color:${ANSI_FG_BRIGHT[c - 90]};`;
...
function extractToolInput(name, inputObj) {
  ...
  switch (name) {
    ...
    case "Task":
      return inputObj.prompt ? inputObj.prompt.slice(0, 120) : "";
    case "TodoWrite":
      return inputObj.todos ? `${inputObj.todos.length} items` : "";
    default: {
      // Try common keys
...
function renderPrettyLogs(rawBuffer) {
  ...
  for (const line of lines) {
    ...
    if (!evt) {
      ...
      if (trimmed) {
        blocks.push(
          `<div class="cc-block cc-stderr">${ansiToHtml(line)}</div>`,
        );
      }
      continue;
    }
    ...
    if (evt.type === "assistant" && evt.message && evt.message.content) {
      for (const block of evt.message.content) {
        if (block.type === "text" && block.text) {
          ...
          const inputHtml = input
            ? `(<span class="cc-tool-input">${escapeHtml(input.length > 200 ? input.slice(0, 200) + "\u2026" : input)}</span>)`
            : "";
          blocks.push(
            `<div class="cc-block cc-tool-call"><span class="cc-marker">&#x23FA;</span> <span class="cc-tool-name">${escapeHtml(block.name)}</span>${inputHtml}</div>`,
          );
        }
      }
      ...
      for (const block of evt.message.content) {
        ...
        if (!text) {
          blocks.push(
            `<div class="cc-block cc-tool-result"><span class="cc-result-pipe">&#x23BF;</span> <span class="cc-result-empty">(No output)</span></div>`,
          );
          continue;
        }
        ...
        if (resultLines.length > 5) {
          const preview = resultLines
            ...
            .map((l) => escapeHtml(l))
            .join("\n");
          const rest = resultLines
            .slice(3)
            .map((l) => escapeHtml(l))
            .join("\n");
          const remaining = resultLines.length - 3;
          blocks.push(
            `<div class="cc-block cc-tool-result"><span class="cc-result-pipe">&#x23BF;</span> <pre class="cc-result-text">${preview}</pre><details class="cc-expand"><summary class="cc-expand-toggle">+${remaining} lines</summary><pre class="cc-result-text">${rest}</pre></details></div>`,
          );
        } else {
          blocks.push(
            `<div class="cc-block cc-tool-result"><span class="cc-result-pipe">&#x23BF;</span> <pre class="cc-result-text">${escapeHtml(text)}</pre></div>`,
          );
        }
      }
      ...
      if (evt.result) {
        blocks.push(
          `<div class="cc-block cc-final-result"><span class="cc-marker cc-marker-result">&#x23FA;</span> <span class="cc-result-label">[Result]</span> ${escapeHtml(evt.result)}</div>`,
        );
      }
    }
  }
...
function renderOversightInLogs() {
      ...
      apiGet("/api/tasks/" + id + "/oversight")
        ...
        .catch(function () {
          ...
          if (currentTaskId === id && logsMode === "oversight") {
            logsEl.innerHTML =
              '<div class="oversight-error">Failed to load oversight summary.</div>';
          }
        });
    }
    logsEl.innerHTML =
      '<div class="oversight-loading">Fetching oversight summary\u2026</div>';
    return;
  }
  ...
  switch (oversightData.status) {
    case "pending":
      logsEl.innerHTML =
        '<div class="oversight-loading">Oversight summary not yet generated.</div>';
      break;
    case "generating":
      logsEl.innerHTML =
        '<div class="oversight-loading">Generating oversight summary\u2026</div>';
    ...
    case "failed":
      logsEl.innerHTML =
        '<div class="oversight-error">Oversight generation failed' +
        (oversightData.error ? ": " + escapeHtml(oversightData.error) : "") +
        "</div>";
      break;
    case "ready":
      logsEl.innerHTML =
        '<div class="oversight-view">' +
        renderOversightPhases(oversightData.phases) +
        "</div>";
      break;
    default:
...
function renderTestOversightInTestLogs() {
      ...
      apiGet("/api/tasks/" + id + "/oversight/test")
        ...
        .catch(function () {
          ...
          if (currentTaskId === id && testLogsMode === "oversight") {
            logsEl.innerHTML =
              '<div class="oversight-error">Failed to load test oversight summary.</div>';
          }
        });
    }
    logsEl.innerHTML =
      '<div class="oversight-loading">Fetching test oversight summary\u2026</div>';
    return;
  }
  ...
  switch (testOversightData.status) {
    case "pending":
      logsEl.innerHTML =
        '<div class="oversight-loading">Test oversight summary not yet generated.</div>';
      break;
    case "generating":
      logsEl.innerHTML =
        '<div class="oversight-loading">Generating test oversight summary\u2026</div>';
      setTimeout(function () {
    ...
    case "failed":
      logsEl.innerHTML =
        '<div class="oversight-error">Test oversight generation failed' +
        (testOversightData.error
          ? ": " + escapeHtml(testOversightData.error)
          : "") +
        "</div>";
      break;
    case "ready":
      logsEl.innerHTML =
        '<div class="oversight-view">' +
        renderOversightPhases(testOversightData.phases) +
        "</div>";
      break;
    default:
      logsEl.innerHTML = '<div class="oversight-loading">Loading\u2026</div>';
...
function startTestLogStream(id) {
  const task = tasks.find((t) => t.id === id);
  testLogsMode =
    task && (task.status === "done" || task.status === "waiting")
      ? "oversight"
      : "pretty";
  testOversightData = null;
  testOversightFetching = false;
