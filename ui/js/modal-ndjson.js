  ...
  for (const line of lines) {
    ...
    if (evt.type === "assistant" && evt.message && evt.message.content) {
      ...
      if (item.type === "agent_message" && item.text) {
        blocks.push(
          `<div class="cc-block cc-text"><span class="cc-marker">&#x23FA;</span> ${escapeHtml(item.text)}</div>`,
        );
        continue;
      }
      ...
      const commandHtml = command
        ? `(<span class="cc-tool-input">${escapeHtml(command.length > 200 ? command.slice(0, 200) + "\u2026" : command)}</span>)`
        : "";
      const shouldRenderCall =
        evt.type === "item.started" ||
        !itemId ||
        !codexSeenCommandStarts.has(itemId);
      if (shouldRenderCall) {
        blocks.push(
          `<div class="cc-block cc-tool-call"><span class="cc-marker">&#x23FA;</span> <span class="cc-tool-name">Bash</span>${commandHtml}</div>`,
        );
      }
